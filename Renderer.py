...
class FbxModel(Model3D):
    ...
    def __del__(self):
        """
        Clean up PyAssimp resources when the object is deleted.
        """
    ...
    def render(self):
        """
        Render the FBX model using PyAssimp and OpenGL.
        """
        for mesh in self.scene.meshes:
            # Set material properties if available
            if mesh.materialindex >= 0:
                material = self.scene.materials[mesh.materialindex]
                
                # Set diffuse color
                if 'diffuse' in material.properties:
                    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, material.properties['diffuse'])
                
                # Set ambient color
                if 'ambient' in material.properties:
                    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, material.properties['ambient'])
                
                # Set specular color
                if 'specular' in material.properties:
                    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, material.properties['specular'])
                
                # Set shininess
                if 'shininess' in material.properties:
            ...
            for face in mesh.faces:
                for vertex_index in face:
                    ...
                    if mesh.texturecoords.size > 0:
                        glTexCoord2fv(mesh.texturecoords[0][vertex_index])
                    
                    # Set vertex
                    glVertex3fv(mesh.vertices[vertex_index])
            glEnd()
...
class Renderer:
    ...
    def render_frame(self, frame: Frame, occlusion_mask: np.ndarray, 
        ...
        alpha_mask[occlusion_mask == 0] = 255  # Areas not occluded
